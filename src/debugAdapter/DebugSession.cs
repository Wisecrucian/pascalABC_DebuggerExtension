...
namespace VSCodeDebug
{
	...
	public class SetDBreakpointsResponseBody : ResponseBody
	{
		public Debugger.Breakpoint[] breakpoints { get; }

		public SetDBreakpointsResponseBody(List< Debugger.Breakpoint> bpts = null) {
			if (bpts == null)
				breakpoints = new  Debugger.Breakpoint[0];
			else
				breakpoints = bpts.ToArray< Debugger.Breakpoint>();
		}
	}
	...
	public abstract class DebugSession : ProtocolServer
	{
		...
		protected override void DispatchRequest(string command, dynamic args, Response response)
		{
			...
			try
			{
				if ((string)args["program"] != null)
				{
					// Получение значения поля 'program'
					string programValue = args["program"].ToString();
			
					// Замена '.pas' на '.exe' (если строка заканчивается на '.pas')
					if (programValue.EndsWith(".pas"))
					{
						args["program"] = programValue.Replace(".pas", ".exe");
					}
				}
				if ((object)args["source"] != null && (string)args["source"]["path"] != null)
				{
					// Получение значения поля 'program'
					string programValue = args["source"]["path"].ToString();
				
					// Замена '.pas' на '.exe' (если строка заканчивается на '.pas')
					if (programValue.EndsWith(".pas"))
					{
						args["source"]["path"] = programValue.Replace(".pas", ".exe");
					}
				}
				if ((object)args["source"] != null && (string)args["source"]["name"] != null)
				{
					// Получение значения поля 'program'
					string programValue = args["source"]["name"].ToString();
				
					// Замена '.pas' на '.exe' (если строка заканчивается на '.pas')
					if (programValue.EndsWith(".pas"))
					{
						args["source"]["name"] = programValue.Replace(".pas", ".exe");
					}
				}
			}
			catch (Exception ex)
